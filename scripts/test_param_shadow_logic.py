...
# ParseCmd is ported once, in test_cmd_parse.py, and imported here. Two
# hand-copied ports of the same ST method would drift the moment one is fixed.
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
from test_cmd_parse import NO_CMD, parse_cmd  # noqa: E402
...
class Fb:
    ...
    def handle_reply(self, pend, rx):
        cmd_seen, cmd = False, NO_CMD

        if self.bStateUnanswered:
            self.bStateUnanswered = False
            self.log_event("INFO", "STATE replies resumed")

        # parse_cmd is the shared port of FB_RobotTcpClient.ParseCmd; it returns
        # NO_CMD (-1) when the reply carries no usable frame, which is what keeps
        # an ACK or a SYNC from clobbering a command MAIN has not consumed yet.
        cmd = parse_cmd(rx)
        cmd_seen = cmd >= 0
        if cmd_seen:
            self.nRobotCmd = cmd
